...
export default{
  ...
  methods: {
    ...
    fixList () {
      var cloneNode
      var firstItem = this.$refs.container.firstElementChild
      ...
      this.$nextTick(() => {
        // 用总高度/length，减小较小
        this.height = this.$refs.container.offsetHeight / (this.length + 1) * this.transformNum
        this.transformHeight = this.$refs.container.offsetHeight / (this.length + 1)
    ...
    start () {
      ...
      setInterval(() => {
        ...
        if (this.direction === 'up') {
          currenTranslateY = -this.currentIndex * this.transformHeight + 'px'
        } else {
          currenTranslateY = -(this.currentIndex + 1) * this.transformHeight + 'px'
        }
        this.setTransform(this.$refs.container, 'translate3d(0,' + currenTranslateY + ',0)')
        // 当滑动到首尾边界替补item时，需即刻跳转到正确item位置
        if (this.currentIndex === this.length - this.transformNum) {
          setTimeout(() => {
            this.quickJump(true)
          }, this.duration)
    ...
    /*
      ...
      */
    quickJump (toFirst) {
      var currenTranslateY
      var currenTransitionTime = 'transform 0ms ease-in-out'
      this.setTransition(this.$refs.container, currenTransitionTime)
      if (toFirst) {
          // 跳转到首个item
        this.currentIndex = 0
